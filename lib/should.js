...
var blockify = function (block) {
  if (typeof block === 'function' && /function\s+\w*\((\w+)\)[\s\S]*\w\.should\.\w+/.test(block.toString())) {
    return function (obj) {
      try {
        block(obj);
      } catch (err) {
    ...
    return function (obj) {
      if (!block.test(obj)) {
        var err = new Error('expected match ' + should.inspect(block));
        return err;
      }
    };
  } else {
    return function (obj) {
      if (!eql(block, obj)) {
        var err = new Error('expected eql ' + should.inspect(block));
        return err;
      }
    };
  }
...
Assertion.prototype = {
  ...
  property: function(name, val, description){
    ...
    if (undefined !== val) {
      ...
      if (err = blockify(val)(this.obj[name])) {
   ...
   * @api public
   */
  properties: function(originalNames) {
    var str
      , names
      , isNamesObj
      , self = this
    ...
    if (typeof originalNames === 'object' && !(originalNames instanceof Array)) {
      isNamesObj = true;
      names = Object.keys(originalNames);
    } else {
      names = originalNames instanceof Array
        ? originalNames
    ...
    if (ok && isNamesObj) {
      names.forEach(function (name) {
        var block = blockify(originalNames[name]);
        var err;
        if (err = block(self.obj[name])) {
          ok = false;
          blockMsg += '\n  ' + name + ': ' + err.message;
        }
      });
    ...
    // message
    str = 'have ' + (len > 1 ? 'properties ' : 'a property ') + str + blockMsg;
  ...
  keys: function(originalKeys){
    ...
    if (typeof originalKeys === 'object' && !(originalKeys instanceof Array)) {
      isKeysObject = true;
      keys = Object.keys(originalKeys);
    } else {
      keys = originalKeys instanceof Array
        ? originalKeys
        : Array.prototype.slice.call(arguments);
    }
    ...
    if (ok && isKeysObject) {
      keys.forEach(function (key) {
        var block = blockify(originalKeys[key]);
        var err;
        if (err = block(self.obj[key])) {
          ok = false;
          blockMsg += '\n  ' + key + ': ' + err.message;
        }
      });
    ...
    // message
    str = 'have ' + (len > 1 ? 'keys ' : 'key ') + str + blockMsg;
